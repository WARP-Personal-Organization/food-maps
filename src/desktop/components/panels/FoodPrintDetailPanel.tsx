...
const FoodPrintDetailsPanel: React.FC<FoodPrintDetailsPanelProps> = ({
  ...
  return (
    ...
    >
      {/* Main image */}
      <div
        className={`relative h-[30vh] w-full flex-shrink-0 transition-all duration-300 ${
      ...
      {/* Content wrapper */}
      <div
        className={`rounded-t-sm -mt-2 relative z-10 bg-white flex-1 overflow-y-auto transition-all duration-400 delay-75 ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
        }`}
      >
        {/* FOODPRINT Label */}
        <div
          className={`pt-4 pb-3 px-6 transition-all duration-300 delay-100 ${
        ...
        {/* Title */}
        <div
          className={`px-6 mb-6 transition-all duration-300 delay-150 ${
            isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"
          }`}
        >
        ...
        <div
          className={`px-6 space-y-6 pb-32 transition-all duration-400 delay-200 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
          ...
          <div className="bg-yellow-50 border-l-4 rounded-xl border-yellow-400 p-4 my-6">
          ...
          {selectedFoodPrint.extendedDescription
            ?.slice(2)
            .map((paragraph, idx) => (
              <p key={idx} className="text-gray-800 text-base leading-relaxed rounded-xl">
                {paragraph}
              </p>
            ))}
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white via-white/95 to-transparent pt-16 p-6 z-10 transition-all duration-400 delay-250 ${
